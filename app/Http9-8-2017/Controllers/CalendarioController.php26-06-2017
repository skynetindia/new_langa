...
//                 }
    ...
    public function show(Request $request)
    {
		...
		if($request->user()->id==0 || $request->tipo==1)
	...
	public function jsondata(Request $request) {
		...
		foreach($event as $evkey=>$evval) {
			...
			$newarray['end']=date('Y-m-d',strtotime($evval->annoFine.'-'.$evval->meseFine.'-'.$evval->giornoFine));
    ...
    public function estimates($request){
        ...
        if($request->user()->id == 0) {
            $estimates = DB::table('quotes')->where('is_deleted', 0)->get();                                           
        }
        else {     
            ...
            $arrORwhere['statiemotivipreventivi.name'] = 'NON CONFERMATO';
            

            $estimates = DB::table('quotes')
            ->Join('statipreventivi', 'statipreventivi.id_preventivo', '=', 'quotes.id')
            ->Join('statiemotivipreventivi', 'statiemotivipreventivi.id', '=', 'statipreventivi.id_tipo')
            ->where($arrwhere)->where($arrwhere)->orWhere($arrORwhere)->get();                
            /*$queries = DB::getQueryLog();
            $last_query = end($queries);
            print_r($last_query);                
            exit;*/
        ...
        foreach ($estimates as $estkey => $estvalue) {
            ...
            $startdate = ""; 
            if($request->user()->dipartimento === 2 || $request->user()->dipartimento === 1 || $request->user()->id == 0) {                 
