...
class QuotationPDF extends FPDF
{
	private $quotation;
	private $ente;
	function __construct($preventivo, $ente, $ente_DA,$utente, $orientation='P', $unit='mm', $size='A4') {
		...
		$this->responsabile = DB::table('users')
									...
									->first();
		$this->corpofattura = json_decode(json_encode(DB::table('optional_preventivi')
		...
		$dipartimento = DB::table('departments')
					...
					->first();
		$this->userDepartment = DB::table('departments')
					->where('id', $this->utente->dipartimento)
					->first();
		$this->index = array(
			'qta',
			'oggetto',
	...
	private function writeTotali($quotation) {
		...
		$sconto = $quotation->scontoagente . '% + ' . $quotation->scontobonus . '% ( ' . chr(128) . ' '.number_format($valoresconto1,2) . ' + ' . chr(128) . ' ' .number_format($valoresconto2,2) . ' )'." ";
		$data = array(
	...
	private function writeDatiAzienda($user){
		...
		$datiAziendaFontSize = 5;
		$this->SetFont(self::FONT_NAME,'',8);
		
		
		/*Via Aldo Moro, 6, Alba, CN, Italia */
		//$this->Cell(0,19, $this->userDepartment->indirizzo, self::BORDER4DEV, 1);
		/*$this->Cell(0,2," P.iva: " . $this->userDepartment->piva, self::BORDER4DEV, 1);
		$this->Cell(0,6," Numero REA CN - " . $this->userDepartment->cf, self::BORDER4DEV, 1);
		$this->Cell(0,5," e-mail: " . $this->userDepartment->email." | web: www.langa.tv", self::BORDER4DEV, 1);
		$this->Cell(0,2," mobile: " . $this->userDepartment->telefonodipartimento, self::BORDER4DEV, 1);*/
	...
	private function writeMasterDataView($label, $object, $ln, $leftMargin){
		...
		$logoUrl = url('/storage/app/images/').'/'.$object->logo;
		...
		$logoMaxHeight = 17;
			...
			//$logoUrl="http://localhost/langa_repo/easy/storage/app/images/Admin%20LANGA.jpg";
	...
	function Footer()
	{
		...
		$x = $this->GetX();
		$y = $this->GetY();
		$this->MultiCell(100, 3, "MATERIALE PROTETTO DA COPYRIGHT LANGA WEB INFORMATICA. FIRMANDO QUESTO DOCUMENTO SI ACCETTA/AUTORIZZA LANGA WEB INFORMATICA AD INIZIARE I PROCESSI TECNICI/AMMINISTRATIVI/COMMERCIALI APPLICANDO I METODI DI LAVORO LANGA. INFO SU    www.langa.tv                                   Created by Easy LANGA", self::BORDER4DEV, "R");
		$this->SetY($y);
		$this->SetX($x+100);
		$this->SetFontSize(10);
