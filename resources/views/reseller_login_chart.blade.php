...
$region[]=array(
        ...
        "region_code"=> "lom", "region_color"=> "rgb(11, 104, 170)", 
);
$region[]=array(
        "region_name"=> "Campania",
        "region_code"=> "cam", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Lazio",
        "region_code"=> "laz", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Sicilia",
        "region_code"=> "sic", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Veneto",
        "region_code"=> "ven", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Emilia-Romagna",
        "region_code"=> "emi", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Piemonte",
        "region_code"=> "pie", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Puglia",
        "region_code"=> "pug", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Toscana",
        "region_code"=> "tos", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Calabria",
        "region_code"=> "cal", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Sardegna",
        "region_code"=> "sar", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Liguria",
        "region_code"=> "lig", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Marche",
        "region_code"=> "mar", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Abruzzo",
        "region_code"=> "abr", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Friuli-Venezia Giulia",
        "region_code"=> "fri", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Trentino-Alto Adige",
        "region_code"=> "tre", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Umbria",
        "region_code"=> "umb", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Basilicata",
        "region_code"=> "bas", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Molise",
        "region_code"=> "mol", "region_color"=> "rgb(11, 104, 170)", 
 );
$region[]=array(
        "region_name"=> "Val d'Aosta",
        "region_code"=> "vao", "region_color"=> "rgb(11, 104, 170)", 
 );


foreach ($resellerdetails as $key => $value) {
    $code = strtolower($value->state);
    $code = substr($code, 0, 3);
    $color = (isset($value->color) && !empty($value->color)) ? $value->color : "#3CABFF";
    foreach($region as $rkey => $rval){
        if($rval['region_code'] == $code) {
            $rval['region_name'] = ucfirst($value->name).'<br>'.trans('messages.keyword_phone').': '.$value->cellulare.'<br>'.trans('messages.keyword_email').': '.$value->email;
            $rval['region_color'] = $color;            
        }   
        $region[$rkey]=$rval;
    }
    /*$region[]=array(
            "region_name"=> ucfirst($value->name),
            "region_code"=> $code, "region_color"=> $color, 
     );*/    
}
...
var json_data = JSON.stringify(<?php echo $string;?>);
...
jQuery("g").each(function() {
        ...
        for(var i=0;i<json_data.length;i++) {
            if(json_data[i].region_code == jQuery(this).attr("id")){                
                jQuery(this).attr("fill",json_data[i].region_color);
                jQuery(this).attr("title",json_data[i].region_name);             
            }            
        }
});

    jQuery(document).on("mousemove","g",function(e){
       // jQuery("g").removeAttr("fill");
        var cid = jQuery(this).attr("id");
        for(var i=0;i<json_data.length;i++) {
            if(json_data[i].region_code == jQuery(this).attr("id")){
                jQuery(this).attr("fill",json_data[i].region_color);
        ...
        jQuery('#myToolTip').css('left',left);
        jQuery('#myToolTip').html(tip).fadeIn(10);
    });
    
    jQuery(document).on("mouseout","#Livello_1",function(e){
        jQuery('#myToolTip').fadeOut(10);
    });
    
    jQuery(document).on("click","g",function(){
        //jQuery("g").removeAttr("fill");
        var cid = jQuery(this).attr("id");
        for(var i=0;i<json_data.length;i++)
        {
...
<div class="panel panel-default">
    <div class="panel-body">
        <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id" data-show-refresh="true"  data-show-columns="true" data-url="<?php echo url('dashboard/getreseller/json'); ?>" data-classes="table table-bordered" id="table">
          <thead>
            ...
            <th data-field="city" data-sortable="true">{{trans('messages.keyword_sector')}}</th>
            <th data-field="cellulare" data-sortable="true">{{trans('messages.keyword_phone')}}</th>
            <th data-field="email" data-sortable="true">{{trans('messages.keyword_email')}}</th>
            <th data-field="city" data-sortable="true">{{trans('messages.keyword_address')}}</th>            
           </thead>
        </table>
    </div>
</div>