...
<script type="text/javascript">
 var jqc = jQuery.noConflict();
	jqc( document ).ready(function() {
	    jqc(".setsize").each(function() {
	        jqc(this).height(jqc(this).width());
	    });
	});
	jqc(window).on('resize', function(){
	    jqc(".setsize").each(function() {
	        jqc(this).height(jqc(this).width());
	    });
	});
...
<?php /*<div class="col-md-12">
	<h1>{{trans('messages.keyword_editproject')}} <?php echo '::' . $progetto->id . '/' . substr($progetto->datainizio, -2) ?></h1><hr>
            ...
            <div class="form-group">
	            ...
	            <table class="table table-bordered">
	                ...
	                <tbody id="lavorazioni">
	                    ...
	                    @foreach($lavorazioni as $partecipante)
                           ...
                           <input id="process{{$p}" class="knob" name="percentvalue[]" value="<?php if(isset($partecipante->completamento) && $partecipante->completamento != ""){ echo $partecipante->completamento;} else { echo '0';}?>" data-width="40%" data-skin="tron" data-thickness=".15" data-fgcolor="#FF851b" data-displayprevious="true" >
                          <?php /* <div class="c100 <?php if(isset($partecipante->completamento) && $partecipante->completamento != ""){ echo 'p'.$partecipante->completamento;} else { echo 'p0';}?> small boxs counter_<?php echo $partecipante->id;?>" data-name="counter_<?php echo $partecipante->id;?>">
							...
							<!-- </section> -->
	                            ...
	                            <script>
	                                jqc('.selezione').on("click", function() {
                				selezioneLavorazioni[nLav] = jqc(this).parent().parent();
				                nLav++;
                			});
                			jqc('#impedisc<?php echo $p; ?>').bind("click", function() {
                				this.blur();
                				this.value = "<?php echo $partecipante->programmato ?>";
                			});                			
                			<?php $p++; ?>							
							<?php /*var box=jqc(".box_<?php echo $partecipante->id;?>");
							var bar=jqc(".box_<?php echo $partecipante->id;?> .bar");
							var boxCenter=[box.offset().left+box.width()/2, box.offset().top+box.height()/2];
							
							jqc('.box_<?php echo $partecipante->id;?>').mousemove(function(e){								
								if( jqc("#hoverover_<?php echo $partecipante->id;?>").val()==0) 
								{   
									$old=jqc("#percent-value_<?php echo $partecipante->id;?>").text();
									per=0;    
									...
									jqc("#percent-value_<?php echo $partecipante->id;?>").html(per);
							...
							jqc('.box_<?php echo $partecipante->id;?>').click(function(e){
								//alert('hii');
								if( jqc("#hoverover_<?php echo $partecipante->id;?>").val()==0) 
								jqc("#hoverover_<?php echo $partecipante->id;?>").val(1);
								else
							  jqc("#hoverover_<?php echo $partecipante->id;?>").val(0);
							});*/?>
	                            </script>
	                        </tr>
	                    @endforeach
                 ...
                 jqc('#aggiungiLavorazione').on("click", function() {

                    var countlavorazioni = jqc('#lavorazioni').children('tr').length;
					...
					circles.value = 0;
					circles.setAttribute('data-width', '40%') ;
            		...
            		var appunti = document.createElement("td");	
            			...
            			var input = document.createElement("input");
                			...
                			*/
                			...
                			//jqc("#datepicker" + kLav).datepicker();
                			jqc('.selezione').on("click", function() {
                				selezioneLavorazioni[nLav] = jqc(this).parent().parent();
				                nLav++;
							});
                			jqc('#impedisci' + kLav).bind("click", impedisciModifica);
                			kLav++;
							progressmove();newfun();
			    ...
			    jq(".setsize").each(function() {
			        jq(this).height(jq(this).width());
	                    ...
	                    jqc('#eliminaLavorazione').on("click", function() {
	                       for(var i = 0; i < nLav; i++) {
	                           selezioneLavorazioni[i].remove();
	                       }
	                       nLav = 0;
	                    });
						if(jqc(".boxs").length>0)
							progressmove();							
						function progressmove(){
							var box=jqc(".boxs");
							var bar=jqc(".boxs .bar");
							//var boxCenter=[box.offset().left+box.width()/2, box.offset().top+box.height()/2];
							
							box.mousemove(function(e){
								$class=jqc(this).data('name');
							...
							jqc('.boxs').on('click',function(e){
								...
								jqc("#"+completepercent).val(percomp);
		...
		<div class="col-md-4">
			<div class="form-group"> <div class="height6"></div>
                ...
                jqc('#statoemotivo').on("change", function() {
                    var yourSelect = document.getElementById( "statoemotivo" );
                    document.getElementById("statoemotivo").style.backgroundColor = yourSelect.options[yourSelect.selectedIndex].style.backgroundColor;
                });
                </script>
			<div class="row">            	
                ...
                <div class="col-md-6">
        		    ...
        		    <script>
					  /*jqc( function() {
					    jqc( "#slider-range-max" ).slider({
					      range: "max",
					      min: 0,
					      max: 100,
					      value: {{$progetto->progresso}},
					      slide: function( event, ui ) {
					        jqc( "#amount" ).val( ui.value );
					      }
					    });
					    jqc( "#amount" ).val( jqc( "#slider-range-max" ).slider( "value" ) );
					  } );
  					*/
        		    ...
        		    jqc('#datainizio').datepicker();
        		    jqc('#datafine').datepicker();					
        		    </script>
       <div class="height10"></div>
            ...
            <div class="set-height">
		            <table class="table table-striped table-bordered">
                        ...
                        <?php $k = 0; ?>
							@foreach($noteprivate as $nota)
		                        <tr>
                                    ...
                                    <td>
		                            	<input type="password" name="pass[]" class="form-control" value="{{$nota->password}}">
		                            </td>
                                    <!-- <td>
		                            	<input id="datepicker<?php echo $k;?>" type="text" name="scad[]" class="form-control" value="{{$nota->scadenza}}">
		                            </td> -->
		                            <script>
										jqc("#datepicker<?php echo $k; ?>").datepicker();<?php $k++; ?>
		                                jqc('.selezione').on("click", function() {
	        				                selezioneServizi[nServ] = jqc(this).parent().parent();
	        				                nServ++;
			                	        });
		                            </script>
		                        </tr>
		                ...
		                <script>                
		                    ...
		                    jqc('#aggiungiNote').on("click", function() {
                                var countnoteprivate = jqc('#noteprivate').children('tr').length;
		                       var tab = document.getElementById("noteprivate");
								...
								jqc("#datepicker" + kServ).datepicker();

		                    });

		                    jqc('#eliminaNote').on("click", function() {
			...
			<!-- 	<label for="notetecniche">Note private per il tecnico</label><a onclick="mostraPrivate()" id="mostra"> <i class="fa fa-eye"></i></a>
				...
				<script>
				jqc('#notetecniche').on("click", function() {
					this.blur();
				});
				...
				function mostra() {
					if(jqc('#noteenti').val()) {
						testo = jqc('#noteenti').val();
						jqc('#noteenti').val("");
					} else {
						jqc('#noteenti').val(testo);
						testo = "";
					}
				}
				function mostraPrivate() {
					if(jqc('#notetecniche').val()) {
						testoPrivato = jqc('#notetecniche').val();
						jqc('#notetecniche').val("");
					} else {
						jqc('#notetecniche').val(testoPrivato);
						testoPrivato = "";
					}
				}
				function mostra2() {
	        ...
	        <br> -->
	           ...
	           <!--  <table class="table table-striped table-bordered">
	                ...
	                <tbody id="datisensibili">
	                    @foreach($datisensibili as $dato)
	                        <tr>
	                            ...
	                            <script>
	                                jqc('.selezione').on("click", function() {
        				                selezioneFile2[nDati] = jqc(this).parent().parent();
        				                nDati++;
		                	        });
	                            </script>
	                        </tr>
	                ...
	                <script>
	                    ...
	                    var nDati = 0;
	                    var kDati = 0;
	                    jqc('#aggiungiDati').on("click", function() {
	                    ...
	                    jqc('#eliminaDati').on("click", function() {
	        ...
	        <br>
	            ...
	            <table class="table table-striped table-bordered">
	                ...
	                <script>
	                    ...
	                    var nFile = 0;
	                    var kFile = 0;
	                    jqc('#aggiungiFile').on("click", function() {
	                        ...
	                        tab.appendChild(tr);
	                        jqc('.selezione').on("click", function() {
				                selezioneFile[nFile] = jqc(this).parent().parent();
				                nFile++;
		                	});
	                    });
	                    jqc('#eliminaFile').on("click", function() {
			...
			<label for="partecipanti">Partecipanti</label><br>
	            ...
	            <table class="table table-striped table-bordered">
	                ...
	                <tbody id="partecipanti">
	                    @foreach($partecipanti as $partecipante)
	                        <tr>
	                            ...
	                            <script>
	                                jqc('.selezione').on("click", function() {
				                selezione[n] = jqc(this).parent().parent();
				                n++;
		                	});
	                            </script>
	                        </tr>
	                ...
	                <script>
	                    ...
	                    var n = 0;
	                    var k = 0;
	                    jqc('#aggiungiPartecipante').on("click", function() {
	                        ...
	                        var td = document.createElement("td");
	                        var td1 = document.createElement("td");
	                        var nomeUtente = document.createTextNode(jqc("#utenti option:selected").text());
	                        var idUtente = document.createElement("input");
	                        idUtente.type = "text";
	                        idUtente.className = "form-control";
	                        idUtente.value = jqc("#utenti option:selected").val();
  ...
  <?php echo Form::close(); ?> 
        ...
        jqc(".dropzone").each(function() {
          jqc(this).dropzone({
          complete: function(file) {
            if (file.status == "success") {
               $.ajax({url: urlgetfile, success: function(result){                
                    jqc("#files").html(result);
                    jqc(".dz-preview").remove();
                    jqc(".dz-message").show();
              }});
            }
            if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                jqc( "#addMediacommnetmodal" ).modal();
                jqc('#addMediacommnetmodal').on('shown.bs.modal', function(){                    
                });
            }
          }
          });
        ...
        function deleteQuoteFile(id){
          var urlD = '<?php echo url('/progetti/deletefiles/'); ?>/'+id;
            $.ajax({url: urlD, success: function(result){
              jqc(".quoteFile_"+id).remove();
              }});
        }
        function updateType(typeid,fileid,checkboxid1){           
            var ischeck = 0;            
            if(jqc('#'+checkboxid1+':checkbox:checked').length > 0){                
                var ischeck = 1;
            }
         var checkValues = jqc('input[name=rdUtente_'+fileid+']:checked').map(function()
            {
                return jqc(this).val();
            }).get();
                ...
                echo $html .='</td></tr>';
                    ...
                    var $ = jQuery.noConflict();
                        ...
                        jqc('#aggiungiFile').on("click", function() {
                            ...
                            tab.appendChild(tr);
                            jqc('.selezione').on("click", function() {
                                selezione[nFile] = jqc(this).parent().parent();
                                nFile++;
                            });
                        });
                        jqc('#eliminaFile').on("click", function() {
...
jqc(document).ready(function() {
    ...
    jqc("#project_edit").validate({            
      ...
      jqc(function(){
        ...
        jqc('#commnetform').on('submit',function(e){
            $.ajaxSetup({
                header:jqc('meta[name="_token"]').attr('content')
            })
            e.preventDefault(e);
                $.ajax({
                type:"POST",
                url:'{{ url('/progetti/mediacomment/').'/'.$mediaCode }}',
                data:jqc(this).serialize(),
                //dataType: 'json',
                success: function(data) {                    
                    if(data == 'success'){
                         $.ajax({url: urlgetfile, success: function(result){                
                      ...
                      jqc('#addMediacommnetmodal').modal('hide');
                    }
                },
                error: function(data){                   
                  if(data == 'success'){
