...
<div class="col-md-12">
    <div class="col-md-8">
        <div class="btn-group">
            <h3 style="display:inline"><a href="javascript:changeYear('{{($year - 1)}}','{{$userid}}')"><i class="fa fa-arrow-left"></i>{{$year - 1}}</a></h3>
            <h3 style="display:inline;color:#f37f0d"> {{$year}} </h3><h3 style="display:inline"><a href="javascript:changeYear('{{($year + 1)}}','{{$userid}}')">{{$year + 1}}<i class="fa fa-arrow-right"></i></a></h3>
        </div>
    </div>
...
function changeYear(year,userid){
    $.ajax({
            type:'GET',
            data: {},
            url: '{{ url('api/administration/statistics/') }}/'+userid+'/'+year,
