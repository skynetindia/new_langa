...
<div class="row">
	...
	<div class="col-md-4">
		...
		<select name="statoemotivo" class="form-control" id="statoemotivo" >
			...
			@if($statoemotivoselezionato!=null)
				@foreach($statiemotivi as $statoemotivo)
					<option @if($statoemotivo->id == $statoemotivoselezionato->id_tipo) selected @endif style="background-color:{{$statoemotivo->color}};color:#ffffff" value="{{$statoemotivo->name}}">{{ ucwords(strtolower($statoemotivo->name)) }}</option>
				@endforeach
			@else
				@foreach($statiemotivi as $statoemotivo)
        ...
        <div class="form-group">
			<label for="percentuale">% {{ trans('messages.keyword_total_amount') }}  <span class="required">(*)</span> </label>
        ...
        <div class="form-group"> 
			...
			<div class="form-group">
            <label for="datascadenza"> {{ trans('messages.keyword_expiry_date_invoice') }} <span class="required">(*)</span> </label>
		    <input value="{{$tranche->datascadenza}}" class="form-control" name="datascadenza" id="datascadenza"></input>
            </div>
