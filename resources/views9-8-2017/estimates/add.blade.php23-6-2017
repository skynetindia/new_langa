...
<input type="hidden" id="hdstatoemotivo" name="statoemotivo" value="NON CONFERMATO" />
        ...
        <div class="row">
        	<div class="col-md-12">
            	<div class="form-group">
        <label for="name_preventivo">{{trans('messages.keyword_name_quotation')}}<span class="required">(*)</label>
        <input value="{{ old('oggetto') }}" type="text" id="name_preventivo" name="oggetto" placeholder="{{trans('messages.keyword_name_quotation')}}" class="form-control">
        		</div>
        	</div>
        </div>
        ...
        <div class="row">
                ...
                <div class="col-md-4">
                		<div class="form-group">
                    <label for="idente">{{trans('messages.keyword_to')}}<span class="required">(*)</span></label>
                    <select name="idente" class="js-example-basic-single form-control">
                        <option selected></option>
                        @foreach($enti as $ente)                           
                                <option value="{{$ente->id}}" <?php if( old('idente') == $ente->id){ echo 'selected'; }?>>{{$ente->nomeazienda}}</option>
                        @endforeach
    ...
    $(".js-example-basic-single").select2();
                ...
                <div class="col-md-4">
                    ...
                    <input value="{{old('data')}}" type="text" id="data" name="data" placeholder="{{trans('messages.keyword_date_creation_preventive')}}" class="form-control">
                    	</div>
                </div>
        </div>
    ...
    <script>
        ...
        $('#pacchetti').on("change", function() {
            ...
            for(var i = 0; i < pacchettiLength; i++) {
                ...
                var freq_id = optional[k]['frequenza'];
                // console.log(freq_id);
                ...
                <?php 
                    ...
                    foreach($frequency as $key => $frequencyval){ ?>
                        // var freq_id = optional[k]['frequenza'];
                        var frqid = "<?php echo $frequencyval->id; ?>";
                        // console.log(frqid);
                 
                        if(frqid == freq_id){ console.log("if");
                            <?php $freq .='<option value="'.$frequencyval->id.'" selected>'.$frequencyval->rinnovo. ' '. trans('messages.keyword_days') .'</option>'; ?>
    ...
    <textarea id="considerazioni" name="considerazioni" placeholder="{{trans('messages.keyword_budget_considerations')}}" class="form-control">{{old('considerazioni')}}</textarea>
    			</div>
                <div class="form-group">
                <label for="valenza">{{trans('messages.keyword_valenza')}}<span class="required">(*)</span></label>
    <input value="{{old('valenza')}}" type="text" id="valenza" name="valenza" placeholder="{{trans('messages.keyword_valency_of_the_budget')}}" class="form-control">
    			</div>
    ...
    <input value="{{old('subtotale')}}" step=any type="number" id="subtotale" name="subtotale" placeholder="{{trans('messages.keyword_initial_price')}}" class="form-control" title="{{trans('messages.keyword_initial_value_calculated_individual_packages')}}">
    					</div>
                    </div>
    ...
    <input value="{{old('scontoagente')}}" type="number" step=any id="scontoagente" name="scontoagente" placeholder="{{trans('messages.keyword_agent_discount_-_calculated_on_the_total')}}" class="form-control" title="{{trans('messages.keyword_maximum_discount_attributable')}}">
    			</div>
    ...
    <textarea id="noteimportanti" name="noteimportanti" placeholder="{{trans('messages.keyword_important_notes')}}" class="form-control">{{old('noteimportanti')}}</textarea>
    			</div>
                <div class="form-group">
                <label for="finelavori">{{trans('messages.keyword_end_date_works')}}<span class="required">(*)</span></label>
    <input value="{{old('finelavori')}}" type="text" id="finelavori" name="finelavori" placeholder="{{trans('messages.keyword_date_expected_end_of_work')}}" class="form-control">
    			</div>

    				<div class="clearfix"></div>
    			<div class="row"><div class="col-sm-12"><div class="height73"></div></div></div>
